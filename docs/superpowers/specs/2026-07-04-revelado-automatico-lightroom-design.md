...
3. **Decisión estética (API de Claude, modelo `claude-haiku-4-5` con visión):** recibe la preview reducida + las métricas locales y devuelve JSON con: recorte propuesto (composición, sujeto bien situado), ángulo fino de enderezado, exposición/contraste, y el color como **desviación relativa** (`temp_shift`/`tint_shift`) respecto al balance de blancos de cámara leído del EXIF — la IA es la única dueña del color. Sigue además las preferencias del fotógrafo escritas en `estilo.txt` (se inyectan en cada petición). Salida estructurada y acotada (la app aplica límites de seguridad a todos los valores).
...
5. **Escritura del XMP** (namespace `crs` de Camera Raw, ProcessVersion actual):
   - Globales: `WhiteBalance` ("As Shot" por defecto; solo si la IA pide una desviación significativa se escribe Custom = Kelvin de cámara + desviación; el modo solo-local nunca toca el color), `Exposure2012`, `Contrast2012`, `Highlights2012`, `Shadows2012`, `Whites2012`, `Blacks2012`, `Sharpness`, `LuminanceSmoothing`.
